...
contract User{
    ...
    function registerUser(string memory _username, string memory _password, string memory _name, string memory _authorizeId, string memory _birthDay, string memory _email, uint _role) public {
        require(bytes(usersByUsername[_username].username).length == 0, "Username already exists");


        UserData memory newUser = UserData(nextUserId, _username, _password, _name, _authorizeId,_birthDay,_email, _role);
    ...
    function getUserByUsername(string memory _username) public view returns (uint, string memory, string memory, string memory, string memory, uint,string memory,string memory) {
        UserData memory user = usersByUsername[_username];
        return (user.userId, user.username, "********", user.name, user.authorizeId, user.role, user.birthday, user.email); // Trả về mật khẩu dưới dạng dấu sao
    }
